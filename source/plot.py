...
def plot_correlation(D):
    ...
    for i in range(D.shape[0]):
        for j in range(D.shape[0]):
            corr[i, j] = np.cov(D[i, :], D[j, :]) [0][1] / (np.std(D[i, :] * np.std(D[j, :])))
    plt.figure(figsize=(12,7))
    sns.heatmap(corr, annot=True, fmt='.2f', cmap='Blues', mask=np.triu(corr,+1))
    plt.show()
...
def plot_hist(D, L):
    ...
    for dIdx in range(D.shape[0]):
        ...
        sns.distplot(D1[dIdx, :], ax=axes).set_title(constants.hFea[dIdx]) # Pulsar
...
def plot_scatter(D, L):
    ...
    for dIdx1 in range(D.shape[0]):
        for dIdx2 in range(D.shape[0]):
            ...
            plt.legend()
            plt.tight_layout()  # Use with non-default font size to keep axis label inside the figure
            plt.savefig(f'{constants.PWD}/plots/scatter_%d_%d.pdf' % (dIdx1, dIdx2))
        plt.show()
